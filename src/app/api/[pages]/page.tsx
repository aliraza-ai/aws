...
const InnerPage: React.FC<ContentItem> = ({ params }) => {
  ...
  const handleLanguageChange = (selectedLanguage: "javascript" | "python" | "java") => {
    setLanguage(selectedLanguage);
  };

  const handleCopyClick = () => {
    if (codeBlock.current) {
      const selectedItem = contentApisData.find((item) => item.title === params.pages);
      if (selectedItem) {
            ...
            <div key={item.id}>
              ...
              {item.details.map(e => (
                <div key={e.heading} className="mt-6">
                  ...
                  {/* Method + Route */}
                  <>
                    <span className={`${e.method === 'GET' ? 'text-green-500' : e.method === 'POST' ? 'text-orange-400' : ''} font-thin`}>
                      {e.method}
                    </span>
                    {" "}
                  ...
                  {/* Paragraph */}
                  {e.description && <p className="font-thin">{e.description}</p>}

                  {e.hasBullets ? <ul className="list-disc pl-5">
                    {e.bullets.map(bullet => (
                      <li key={bullet.paramTitle}>
                        <span className="text-purple-500 mr-1">{bullet.paramTitle}</span>
                        {" "}
                        <span className="font-thin opacity-70 mr-1">({bullet.type})</span>
                        {" "}
                        <span className="font-thin">{bullet.description}</span>
                      </li>
                    ))}
                  </ul> : ''}

                  {e.hasCode ?
                    <SyntaxHighlighter language={'json'} style={coldarkDark} showLineNumbers={true} className="custom-scrollbar rounded-lg drop-shadow-xl">
                      {e.code}
                    </SyntaxHighlighter> : ''
                  }

                  {e.hasStatus ? <ul className="list-disc pl-5">
                    {e.bulletStatus.map(bullet => (
                      <li key={bullet.status}>
                        <span className={`${bullet.status === '200 OK' ? 'text-blue-500' : bullet.status === '400 Bad Request' ? 'text-red-500' : ''} mr-1`}>{bullet.status}</span>
                        {": "}
                        <span className="font-thin">{bullet.description}</span>
                      </li>
                    ))}
                  ...
                  {e.hasUsage ? <ul className="list-disc pl-5">
              ...
              <div className="relative rounded-lg overflow-hidden py-6">
                ...
                <div className="flex items-center justify-end">
                  <button onClick={handleCopyClick} className="copyButton self-end bg-gray-700  text-white rounded-full text-sm px-2 pt-0.5 pb-1 -mb-[46px] mr-2">
                    {copyButtonText}
                  </button>
                </div>

                <div ref={codeBlock} className="bg-[#111B27] pt-5 rounded-b-lg overflow-hidden">
                  <SyntaxHighlighter
                    language={language}
                    style={coldarkDark}
