                ...
                <ul className="text-left mt-5 md:mt-0">
                    {sidebarLinks.map((item) => (
                        <li key={item.id} onClick={() => setApiSidebartoggle(false)}>
                            <Link href={item.link}>
                                <div className={`flex gap-3 w-full p-2 hover:bg-[#640f6c] items-center transition-all text-white duration-200 sm:text-base text-sm rounded-md ${pathName == item.link ? "bg-[#640f6c]" : ""}`}>
                                    <span>
